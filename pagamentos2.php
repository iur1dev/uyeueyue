...
<body>
    <div class="container">
        ...
        <div class="card text-dark my-3 mx-auto text-center" style="width: 18rem;">
            <div class="card-body">
                <?php
                while ($row = mysqli_fetch_assoc($query_total_cesta)) {
                    echo '<h5 class="card-title">Total a Receber R$ ' . number_format($row['total'], 2, ",", ".") . '</h5>';
                }
                ?>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_assoc($query)) {
                        ...
                        if ($row['id_metodo_pagamento'] == 1) {
                            echo '<p>1ª Parcela R$ ' . number_format($row['valor_cesta'], 2, ",", ".")  . '</p>';
                        } else if ($row['id_metodo_pagamento'] == 2) {
                            $duas_vezes = $row['valor_cesta'] / 2;
                            echo '<p>1ª Parcela R$ ' . number_format($duas_vezes, 2, ",", ".") . '</p>';
                            echo '<p>2ª Parcela R$ ' . number_format($duas_vezes, 2, ",", ".") . '</p>';
                        } else if ($row['id_metodo_pagamento'] == 3) {
                            $tres_vezes = $row['valor_cesta'] / 3;
                            echo '<p>1ª Parcela R$ ' . number_format($tres_vezes, 2, ",", ".") . '</p>';
                            echo '<p>2ª Parcela R$ ' . number_format($tres_vezes, 2, ",", ".") . '</p>';
                            echo '<p>3ª Parcela R$ ' . number_format($tres_vezes, 2, ",", ".") . '</p>';
                        } else if ($row['id_metodo_pagamento'] == 4) {
                            $quatro_vezes = $row['valor_cesta'] / 4;
                            echo '<p>1ª Parcela R$ ' . number_format($quatro_vezes, 2, ",", ".") . '</p>';
                            echo '<p>2ª Parcela R$ ' . number_format($quatro_vezes, 2, ",", ".") . '</p>';
                            echo '<p>3ª Parcela R$ ' . number_format($quatro_vezes, 2, ",", ".") . '</p>';
                            echo '<p>4ª Parcela R$ ' . number_format($quatro_vezes, 2, ",", ".") . '</p>';
                        }
                        echo '</td>';
                        ...
                        echo '<td>';
                        if ($row['id_metodo_pagamento'] == 1) {
                            echo '<input type="number" class="form-control text-center" name="v1" placeholder="Digite o Valor">';
                        } else if ($row['id_metodo_pagamento'] == 2) {
                            $duas_vezes = $row['valor_cesta'] / 2;
                            echo '<input type="number" class="form-control text-center" name="v1" placeholder="Digite o Valor">';
                            echo '<input type="number" class="form-control text-center" name="v2" placeholder="Digite o Valor">';
                        } else if ($row['id_metodo_pagamento'] == 3) {
                            $tres_vezes = $row['valor_cesta'] / 3;
                            echo '<input type="number" class="form-control text-center" name="v1" placeholder="Digite o Valor">';
                            echo '<input type="number" class="form-control text-center" name="v2" placeholder="Digite o Valor">';
                            echo '<input type="number" class="form-control text-center" name="v3" placeholder="Digite o Valor">';
                        } else if ($row['id_metodo_pagamento'] == 4) {
                            $quatro_vezes = $row['valor_cesta'] / 4;
                            echo '<input type="number" class="form-control text-center" name="v1" placeholder="Digite o Valor">';
                            echo '<input type="number" class="form-control text-center" name="v2" placeholder="Digite o Valor">';
                            echo '<input type="number" class="form-control text-center" name="v3" placeholder="Digite o Valor">';
                            echo '<input type="number" class="form-control text-center" name="v4" placeholder="Digite o Valor">';
                        }
                        echo '</td>';

                        echo '<td>';
                        echo "<button type=submit><i class='fa-sharp fa-solid fa-coins fs-4 text-warning'></i></button>";
                        echo '</td>';
